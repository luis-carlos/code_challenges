...
var findPivotIndex = function(numberOfElements, minValue, maxValue) {
    let leftSum = 0;
    let randomArray = getRandomArray(numberOfElements, minValue, maxValue);
    const sum = randomArray.reduce(function (result, item) {
       return result + item;
    });
    console.log(randomArray);
    for(let i = 0; i < randomArray.length; i++) {
        if (leftSum == sum - leftSum - randomArray[i])
            return i;
        leftSum += randomArray[i];
    }
    return -1;
...
console.log(findPivotIndex(process.argv[2], process.argv[3], process.argv[4]));